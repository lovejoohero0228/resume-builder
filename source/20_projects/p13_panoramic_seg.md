---
id: p13
title_ko: Dental Panoramic Radiograph 분할·검출 (10여 개 모델)
...
facts:
  f1: {kind: scope, value_ko: "치과 파노라마에서 치아(Tooth)·상악동(sinus)·하치조신경관(IAC)·치조정(Alveolar crest)·CEJ·보철물(Prosthesis)·악관절(TMJ) 분할과 우식증(Caries)·치주염(periodontitis)·매복치(Impaction)·방사선투과상(Radiolucency)·병변(Lesion) 탐지 등 10여 개 모델을 개발", value_en: "developed ~10 models on dental panoramic radiographs — segmentation of Tooth, sinus, IAC, Alveolar crest, CEJ, Prosthesis, and TMJ, plus detection of Caries, periodontitis, Impaction, Radiolucency, and Lesion", disclosure: public, confidence: measured}
...
short:
  ko: >
    치과 파노라마에서 치아·상악동·IAC·치조정·CEJ·보철물·TMJ 분할과 우식증·치주염·매복치·방사선투과상·병변 탐지 등
